...
import { useMyApplies } from "@/hooks/queries/useMy";
import { ListItem } from "@/components/common/ListItem";

export const MyHistoryPage = () => {
  const navigate = useNavigate();
  const { data: applies, isLoading, error } = useMyApplies();
  ...
  if (error) {
    return (
      <div className="min-h-screen bg-brand-background text-white flex items-center justify-center">
        <div className="text-center">
          <p className="text-red-400 mb-4">지원 내역을 불러올 수 없습니다.</p>
          <button
          ...
          {applies && applies.length > 0 ? (
            <div className="space-y-4">
              {applies
                ...
                .map((apply: any) => (
                  <ListItem key={apply.id} post={apply} />
                ))}
            </div>
          ) : (
            <div className="text-center py-12">
              <p className="text-brand-text">지원한 모집글이 없습니다.</p>
            </div>
          )}
        </div>
      </div>
